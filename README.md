...
14. **Tag the notification *source* — a notification-listener sees every app alike.** The Android notification trigger fires for an SMS, a messenger group-chat, a bank alert — all indistinguishably. If the macro forwards only `{title, text}`, everything downstream reads as "an SMS" and you can't route or label by channel. Fix: forward the **app name** too (MacroDroid `{not_app_name}`) and classify server-side (`app contains "kakao"/"whatsapp"/… → that channel, else sms`) — backward-compatible, since a missing app field just falls back to SMS. For a messenger, the notification *title* is usually the room/sender name, so capture it as the sender. Cheap, and it lets the memory layer file "a message from the X group" distinctly from a real text.
15. **A call recording carries no caller ID — grab it from the phone, not the audio.** The transcript of a call tells you *what* was said, never *who* called. If you want the number/name filed with it, the phone has to send it: add it as a query param/header on the upload (MacroDroid exposes the caller number/contact-name as magic-text variables). Gotcha we hit: it silently arrived empty until the automation was granted **Call-Log** permission specifically — "Phone" permission alone isn't enough. Verify against the server log, not the macro's success toast.
16. **CJK / agglutinative languages quietly break naive lexical recall.** Substring keyword matching false-positives badly in Korean/Japanese/Chinese: a 1-character token substring-matches inside dozens of unrelated words, and conjugation means the query form never string-matches the stored root. IDF weighting helps but isn't enough — we watched casual chit-chat out-score real work queries because a verbose memory happened to contain common short syllables. What held: drop 1-char tokens, a tiny stopword list, and **calibrate the relevance threshold on real traffic** (casual messages vs. work prompts land in measurably different score bands; a threshold tuned on tidy keyword queries is wrong for natural sentences). Serving a CJK workload? Budget for morphological tokenization or lean harder on the semantic signal — don't trust substring lexical alone.
17. **"Recall before you assert" is a prompt hope — make it a reflex.** The memory contract says consult memory before deciding; per lesson 11, prompt-level discipline drifts. The enforcement version is an automatic pre-prompt hook that runs recall on the *incoming* message and injects the top hits as context — recall goes from "the agent must remember to" to "always on." The catch is noise, so gate it behind the calibrated threshold from lesson 16 and skip trivial/greeting messages; if it injects on every "thanks 🙂" it becomes wallpaper the agent learns to ignore. **Fail-open** — a recall miss or timeout must never block the user's message.
18. **Bi-temporal memory is theater without a verification sweep.** Storing `last_verified` apart from `valid_from` only helps if something actually re-verifies. Measure it: we found ~half our memories had `last_verified == created_at` — never re-checked — so the "freshness" signal was silently guessing for half the corpus. Schedule a sweep that surfaces the oldest-unverified facts for the agent to re-confirm or invalidate, or bi-temporal is a field you write once and then lie with forever.
19. **Plan for the box *itself* dying — off-box, and split-brain-safe.** Everything above assumes the server is up; ours went fully dark once (lesson 10). Three things that matter when it does. (a) An **off-box deadman switch**: a watchdog that lives *somewhere else* — a scheduled CI job, a second machine — and alerts when the server stops checking in. A monitor running *on* the dead box tells you nothing. (b) **Backup = a verified restore, not a green checkmark.** We found a "successful" backup that couldn't actually be restored (a missing directory, a silently-broken job); rehearse the restore end-to-end on a schedule, or you don't have a backup. (c) If you keep a warm standby that can take over, make failover **lease-based** — a single shared lock one side must hold and renew — so two machines can't both decide they're primary; keep failback manual so a human stays in the loop.
